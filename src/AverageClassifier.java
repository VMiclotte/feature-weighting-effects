...
public class AverageClassifier extends AbstractClassifier {
    ...
    public AverageClassifier(Instances trainData, int k) {
    ...
    public double calc(Instance[] n, double classValue, Instance x) {
        double sum = 0;
        double sum2 = 0;
    ...
    public double[] distributionForInstance(Instance instance) throws Exception {
        ...
        for (int i = 0; i < trainData.numClasses(); i++) {
            //dist[i] = (lowApp(n, i, instance) + uppApp(n, i, instance)) /2.0;
            double calc = calc(n, i, instance);
            dist[i] = (low(calc) + upp(calc)) / 2;
        }
        return dist;
    }
    ...
    public double upp(double calc) {
        FQ most = new FQ(0.2, 1);
        double ret = most.fuzQua(calc);
        return ret;
    }

    public double low(double calc) {
        FQ some = new FQ(0.1, 0.6);
        double ret = some.fuzQua(calc);
        return ret;
    }
